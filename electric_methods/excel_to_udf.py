...
df = pd.read_excel('data\electric_data.xlsx',sheet_name=1,index_col=None).drop('n',axis=1)
...
header.to_csv('testing.csv', mode = 'w', index = False,sep='\t')
topo.to_csv('testing.csv', mode = 'a', index = False, header=False,sep='\t')
middle.to_csv('testing.csv', mode = 'a', index = False,sep='\t')
df.to_csv('testing.csv', index=False, mode = 'a', header=False, sep='\t')
