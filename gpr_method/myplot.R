...
plot.GPR <- function(x, 
    ...
    if(isTRUE(add)){
      ...
      if(is.null(dots$ann) || dots$ann != FALSE){
        if(is.null(dotsxaxt) || dotsxaxt != "n"){
          x_axis <- pretty(z, 10)
          xat <- axis(side = 1, cex.axis = 1.2,  tck = +0.02)
          if(grepl("[m]$", x@depthunit)){
            # axis(side = 3, cex.axis = 1.2, at = x_axis, labels = x_axis, tck = +0.02)
            axis(side = 3, cex.axis = 1.2, tck = +0.02)
            ...
            if( length(x@antsep) > 0 && is.null(dim(v)) && length(v) == 1 && v > 0){
              if( x@antsep > 0){
                ...
                if(max(z)*v/2 > 1.3){
                  ...
                  axis(side = 3, cex.axis = 1.2, at = t0 + depthat, labels = depth, tck = +0.02)
                  #print(t0)
                }
                ...
                mtext(paste0("Profundidad (m),   v=", v, "m/ns"), side = 3, line = 2, cex = 1.5)
              }else{
                depth <- pretty(xat * v / 2, 10)
                depthat <- depthToTime(depth, 0, v, antsep = x@antsep)
                axis(side = 3, cex.axis = 1.2, at = t0 + depthat, labels = depth, tck = +0.02)
  ...
  # Plot 2D ------  
    ...
    xvalues <- x@pos
    yvalues <- x@depth
    myxlab <- paste0("Longitud (", x@posunit, ")")
    ...
    if( grepl("[s]$", x@depthunit) ){
      myylab <- paste0("Tiempo (", x@depthunit, ")")
          ...
          if(is.null(dim(v)) && length(v) == 1 && v > 0 ){
            if(x@antsep > 0){
              ...
              if(max(yvalues) * v / 2 > 1.3){
                ...
                depthat <- depthToTime(depth, 0, v, antsep = x@antsep)
                axis(side = 4, cex.axis = 1.2, at = t0 + depthat, labels = depth, tck = -0.02)
              }
              depthat2 <- depthToTime(depth2, 0, v, antsep = x@antsep)
              axis(side = 4, cex.axis = 1.2, at = t0 + depthat2, labels = FALSE, tck = -0.01)
              axis(side = 4, cex.axis = 1.2, at = depth_0, labels = "0", tick = FALSE)
              if(isTRUE(addDepth0)) abline(h = depth_0, col = "grey", lty = 3)
              # mtext(paste0("depth (m),   v=", v, "m/ns"), side = 4, line = 2)
              mtext(paste0("Profundidad (", x@posunit, "),   v = ", round(v, 3), " ", x@posunit, 
                           "/",  x@depthunit), side = 4, line = 2.5, cex = 1.5)
            }else{
              depth_0 <- t0 + depth0(0, v, antsep = x@antsep)
              depth <- pretty(yat * v / 2, 10)
              depthat <- depthToTime(depth, 0, v, antsep = x@antsep)
...
.barScale <- function(clim, y, col, clab = "mV", clabcex = 0.8){
  ...
  dclim <- clim[2] - clim[1] 
  if(dclim == 0){
    axis(side = 4, cex.axis = 1.2, las = 2, at = (usr[3] - usr[4])/2, labels = clim[1])
    ...
    axis(side = 4, cex.axis = 1.2, las = 2, at = pretty_at, labels = pretty_z)
...
.depthAxis <- function(y, pretty_y, time_0, v, antsep, depthunit, posunit ){
  if(grepl("[s]$",depthunit)){
    ...
    axis(side = 4, cex.axis = 1.2, at = -depthAllPos - time_0, labels = depthAll, tck = -0.02)
    depth2  <- seq(0.1, by = 0.1, 0.9)
    depthat2 <- depthToTime(depth2, 0, v, antsep = antsep)
    axis(side = 4, cex.axis = 1.2, at = - depthat2 - time_0, labels = FALSE, tck = -0.01)
    mtext(paste0("Profundidad (", posunit, "),   v = ",v, " ", posunit, "/", 
                 depthunit), side = 4, line = 2.5, cex = 1.5)
  }else{
    axis(side = 4, cex.axis = 1.2, at = pretty_y, labels = -pretty_y)
    mtext(paste0("Profundidad (", depthunit, ")") ,side = 4, line = 3, cex = 1.5)
  }
}
